...
export default function StaffSalaryView() {
  const { staff, campuses, paySalary, addStaff, updateStaff, deleteStaff, currentUser, showToast } = useApp();
                ...
                <tr key={st.id}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', alignItems: 'center' }}>
                      ...
                      {/* Delete Staff Icon Button (Super Admin Only) */}
                      {isSuperAdmin && (
                        <button 
                          type="button"
                          className="table-action-icon-btn btn-delete"
                          title="Delete Staff Member (Super Admin Only)"
